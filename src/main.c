...
static void draw_big_battle_card(int id, int x, int y, int back)
{
    ...
    if (back) {
        draw_card_raw(waifu_card_back, WAIFU_CARD_W, WAIFU_CARD_H, x+4, y+6, 112, 112);
    } else {
    ...
    if (!back) {
        char stats[32];
        snprintf(stats, sizeof(stats), "A%u D%u", (unsigned)waifu_card_atk[id], (unsigned)waifu_card_def[id]);
...
static void draw_big_battle_card_rect(int id, int x, int y, int w, int h, int back)
{
    ...
    if (w > 8 && h > 14) {
        ...
        if (art_w < 1) art_w = 1;
        if (art_h < 1) art_h = 1;
        if (back) draw_card_raw(waifu_card_back, WAIFU_CARD_W, WAIFU_CARD_H, x+4, y+6, art_w, art_h);
        else draw_card_raw(big_art_ptr(id), WAIFU_BIG_W, WAIFU_BIG_H, x+4, y+6, art_w, art_h);
    } else {
...
static void draw_board_card_ex(Camera cam, int col, int row, int card_id, int back, int gray)
{
    ...
    float y = 0.115f;
    const uint8_t *tex = back ? waifu_card_back : card_face_ptr(card_id);
                                       ...
                                       BattleOutcome outcome)
{
    ...
    if (local < slide_dur) {
        ...
        draw_big_battle_card(atk_id, ax0, ay, atk_back);
        draw_big_battle_card(def_id, dx0, dy, def_back);
    } else if (def_back && local < reveal_end) {
        draw_big_battle_card(atk_id, ax, ay, atk_back);
        draw_big_battle_card_flip(def_id, dx, dy, local - flip_start, flip_dur);
    } else if (local < ram_start) {
        draw_big_battle_card(atk_id, ax, ay, atk_back);
        draw_big_battle_card(def_id, dx, dy, 0);
        ...
        else draw_big_battle_card(atk_id, atk_x, ay, atk_back);

        if (flash_defender) draw_big_battle_card_hit_flash(def_id, def_x + shake_defender, dy, 0, local);
        else draw_big_battle_card(def_id, def_x, dy, 0);
        ...
        if (outcome != BATTLE_DESTROY_ATTACKER && outcome != BATTLE_DESTROY_BOTH) draw_big_battle_card(atk_id, ax, ay, atk_back);
        if (outcome != BATTLE_DESTROY_DEFENDER && outcome != BATTLE_DESTROY_BOTH) draw_big_battle_card(def_id, dx, dy, 0);
    }
...
static int defender_battle_value(int defender_owner, int slot)
{
    ...
    return faceup ? (int)waifu_card_atk[id] : (int)waifu_card_def[id];
}
...
static void prepare_battle(int attacker_owner, int attacker_slot, int defender_slot)
{
    ...
    if (attacker_owner == 0) {
        ...
        delta = (int)waifu_card_atk[atk_id] - defender_battle_value(1, defender_slot);
    } else {
        ...
        delta = (int)waifu_card_atk[atk_id] - defender_battle_value(0, defender_slot);
    }
...
static void prepare_direct_attack(int attacker_owner, int attacker_slot)
{
    ...
    int dmg = (int)waifu_card_atk[atk_id];
    g_b_battle_atk_owner = attacker_owner;
    g_b_battle_atk_slot = attacker_slot;
    g_b_battle_def_slot = -1;
    g_b_battle_atk_card = atk_id;
...
static void resolve_battle(void)
{
    ...
    if (g_b_battle_outcome == BATTLE_DIRECT_ATTACK) {
        ...
        if (g_b_battle_outcome == BATTLE_DESTROY_DEFENDER) {
            int delta = (int)waifu_card_atk[atk_id] - defender_battle_value(1, g_b_battle_def_slot);
            if (delta < 0) delta = 0;
            g_com_lp -= delta;
            ...
            int delta = defender_battle_value(1, g_b_battle_def_slot) - (int)waifu_card_atk[atk_id];
            if (delta < 0) delta = 0;
            g_you_lp -= delta;
        ...
        if (g_b_battle_outcome == BATTLE_DESTROY_DEFENDER) {
            int delta = (int)waifu_card_atk[atk_id] - defender_battle_value(0, g_b_battle_def_slot);
            if (delta < 0) delta = 0;
            g_you_lp -= delta;
            ...
            int delta = defender_battle_value(0, g_b_battle_def_slot) - (int)waifu_card_atk[atk_id];
            if (delta < 0) delta = 0;
            g_com_lp -= delta;
...
static void draw_direct_attack_event(int f, int atk_id, int atk_col, int atk_row, int atk_back)
{
    ...
    draw_big_battle_card(atk_id, card_x, ay, atk_back);
...
static void step_battle_interactive(const WaifuFmInput *input, int press_up, int press_down, int press_left, int press_right, int press_a, int press_b, int press_start)
{
    ...
    case IB_PLAYER_DRAW:
        draw_interactive_common(player_camera(), g_i_player_hand[g_b_selected_hand], "DRAW");
...
{
    WaifuFmInput zero;
    int press_up, press_down, press_left, press_right, press_a, press_b, press_start;
