...
#ifndef WAIFU_PCFX_ADPCM_RAM_LOAD
#define WAIFU_PCFX_ADPCM_RAM_LOAD 0
#endif

#if WAIFU_PCFX_ADPCM_RAM_LOAD
#include "fastking.h"
#include <eris/king.h>
#endif
...
/* KING PageSetting register layout follows the pcfxemu/HuC6272 behavior used
   by the test target.  DMA selects page 1 with bit 0, BG fetches select page 1
   with bit 4, ADPCM selects page 1 with bit 8, and RAINBOW selects page 1
   with bit 12. */
#define WAIFU_PCFX_KRAM_PAGESETTING_SCSI1   0x00000001u
#define WAIFU_PCFX_KRAM_PAGESETTING_BG1     0x00000010u
#define WAIFU_PCFX_KRAM_PAGESETTING_ADPCM1  0x00000100u
...
#if WAIFU_PCFX_ADPCM_RAM_LOAD
    /* Optional debug fallback: CD -> RAM scratch -> KING KRAM write.
       The normal path below uses direct CD->KRAM DMA and is the shipping path. */
    remaining = cd_round_sector_bytes_u32((uint32_t)bytes);
    lba = BINARY_LBA_ASSETS_GENERATED_SFX_ADPCM_BIN;
    word_addr = kram_addr;
    king_set_page_setting(WAIFU_PCFX_KRAM_PAGESETTING_ADPCM1);
    while (remaining) {
        uint32_t chunk = remaining > PCFX_CD_SECTOR_SIZE ? PCFX_CD_SECTOR_SIZE : remaining;
        cd_read(lba, g_cd_sector_scratch, PCFX_CD_SECTOR_SIZE);
        eris_king_set_kram_write(0x80000000u | word_addr, 1);
        king_kram_write_buffer(g_cd_sector_scratch, (int)chunk);
        word_addr += chunk >> 1;
        remaining -= chunk;
        ++lba;
    }
