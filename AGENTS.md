...
- Big-art (112x112 card art: card-check preview, equip/battle card displays) is blitted into the CPU framebuffer like everything else and goes through the normal dirty-band path. The old `pcfx_block16_is_direct_big_art` optimization (have the dirty scanner *skip* big-art blocks and upload them via a separate `pcfx_upload_direct_big_art_rects`) corrupted KRAM during equip/battle animations and left whole-screen comb garbage that persisted (shadow marked clean while KRAM held garbage); it was removed. `pcfx_upload_direct_big_art_rects` remains but now always self-skips (shadow already synced by the dirty bands).
