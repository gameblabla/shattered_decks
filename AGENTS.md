...
- Story mode:
  ...
  - PC-FX save loading from the backup-device picker enters `WAIFU_I_STORY_LOAD_TO_MAP`: a short black, music-silent transition that clears the title VDC overlay before BackupRAM is read and the first story-map 3D frame is drawn. Do not collapse this back into the load-device A-press frame; title CD-DA / PSG confirm can latch while backup I/O and the initial 3D map render run.
