...
- Story mode:
  ...
  - PC-FX Load Story now picks internal Backup RAM directly, falling back to FX-BMP only if internal has no save, then enters `WAIFU_I_STORY_LOAD_TO_MAP`: a short visible BACKUP RAM LOADING, music-silent transition that clears the title VDC overlay before BackupRAM is read and the first story-map 3D frame is drawn. Do not collapse BackupRAM I/O back into the title/menu A-press frame; title CD-DA / PSG confirm can latch while backup I/O and the initial 3D map render run.
  - The Sanctum menu (`draw_story_pyramid_menu`) uses a right-side panel so the 3D pyramid remains visible on the left. A centered opaque panel regressed the PC-FX view into a mostly black menu because the PC-FX sky path is black and the pyramid sits in the center of the composition.
...
- `WAIFU_I_STORY_LOAD_DEVICE` is retained as a fallback title-backed UI, but the normal Load Story action bypasses it and loads internal Backup RAM directly. If internal has no save, it tries FX-BMP. Entering the row itself should not get stranded on the title overlay.
- Confirm SFX is suppressed for PC-FX internal/external load actions in the backup-device picker; those actions enter `WAIFU_I_STORY_LOAD_TO_MAP` instead. BACK still plays the normal confirm-alt effect.
- The PC-FX title-backed Load Story row accepts a held A/RUN level as well as the normal press edge, but only while that row is selected. The fallback backup-device picker also accepts a held A/RUN after an 8-frame guard. This keeps pcfxemu pad-edge timing from leaving the overlay highlighted or stranded in the device picker.
...
- The PC-FX audio backend also repeats channel-off writes for a few frames after each PSG voice ends, and silent music states force both PSG off and a short CD-DA STOP burst. This covers emulator/state cases where a SoundBox channel or drive playback is latched even though the game-side voice/music state has ended.
