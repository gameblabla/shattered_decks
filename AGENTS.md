...
- [Makefile.cd32x](/home/anonymous/Documents/DEV/Anime_card/waifu_card_game/Makefile.cd32x:1)
  ...
  - CD32X title/menu/loading/game rendering must use software full redraw (`waifu_platform_text_overlay_is_hardware() == 0`) because the 32X CPU-visible framebuffer is the current back page and flips every frame. Do not restore one-shot title/menu hardware overlay composition. Blastem's `32x_video.c` is the page-flip reference: framebuffer writes always hit `video->back`, changing FS swaps `front`/`back`, and `0x24020000` is the overwrite aperture for the same back page, not page 1. `waifu_cd32x_video_present_8bpp()` must write the line table and all pixels to the current back page before every flip; do not use fixed physical page pointers for double buffering.
