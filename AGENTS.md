...
- [Makefile.cd32x](/home/anonymous/Documents/DEV/Anime_card/waifu_card_game/Makefile.cd32x:1)
  ...
  - Small 38x54 card faces on CD32X are streamed per card into a 24-slot LRU (`waifu_assets_card_face`), and the supervisor (`cd32x_service_card_face_request`) must serve them with the same sector-accurate `cd32x_read_file_slice("CARD_FACES.BIN", card_id*WAIFU_CARD_W*WAIFU_CARD_H, …)` + `cd32x_transfer_word_ram_to_32x` path the big-art/portrait reads use. Do NOT go back to `load_file()` (whole atlas) + `memcpy` slide: the BIOS file load lands the data four bytes off the raw `read_cd` user-data window, so every face came out rotated four pixels and the card frame "wrapped around" on the field. `read_cd`-by-LBA keeps faces byte-identical to the on-disc atlas.
