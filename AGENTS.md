...
- [Makefile.cd32x](/home/anonymous/Documents/DEV/Anime_card/waifu_card_game/Makefile.cd32x:1)
  ...
  - CD32X is a 320x224 NTSC target. Keep gameplay layout platform-agnostic by deriving bottom HUDs, hand rows, story/card-check text boxes, and battle cut-in lanes from `WAIFU_FM_WIDTH` / `WAIFU_FM_HEIGHT`; do not add new 240-line assumptions to common gameplay code. The CD32X title sidecar is generated from `assets/source/title/title_320x224.png`, and the CD32X ending sidecar fits `assets/source/ending/ending320x240.png` into a 320x224 external `ENDING_SCREEN_IMG.BIN` loaded through the normal asset request path.
  ...
  - CD32X rendering follows Blastem's `32x_video.c` page model: framebuffer writes always hit `video->back`, changing FS swaps `front`/`back`, and `0x24020000` is the overwrite aperture for the same back page, not page 1. Common CD32X drawing uses the current back page directly as `waifu_fm_framebuffer()`, so `waifu_cd32x_video_present_8bpp()` must not repack-copy that framebuffer onto itself; both pages get their line tables during init/clear, and present normally only updates palette state before the flip. Title/menu use the CD32X hardware-overlay hook again, but the backend warms both flipped pages after each title/menu mode change and restores only the prompt/menu rectangles from the resident title asset; do not return to a one-page title/menu composition, fixed physical page pointers, or per-frame title/menu redraw. CD32X 2D clears, same-size image blits, title/ending restores, and battle/board cache restores use packed 16-bit framebuffer writes with byte stores only for odd left/right edges; do not regress these paths to byte-at-a-time copies. The CD32X 1x title/menu overlay text writes sparse glyph pixels directly as bytes into the back framebuffer; avoid restoring the older per-pixel one-rect path because it stalls the menu.
  - CD32X 3D raster hot paths live in the renderer backend split: `src/engine/renderer3d_cd32x.c` includes `renderer3d_spans.inc` with `CFX_RENDERER_USE_SH1_ASM`, while geometry and edge walking stay in `renderer3d.c`. Keep SH-2 assembly behind renderer port macros so PC-FX/headless keep their existing paths. The battle/story 3D quads use the compact affine renderer on CD32X; avoid falling back to the old per-pixel software triangle mapper unless the visual tradeoff is intentional. CD32X board spans use fixed-point affine stepping with a packed SH-2 halfword fill loop and no expanded texture LUT, so do not replace it with byte stores or a 64 KiB texel cache in SDRAM.
  - CD32X battle board top rendering uses both SH-2s: `src/main.c` sends only board-row raster jobs to the Slave through an uncached SDRAM mailbox serviced from `src/platform/cd32x/cd32x_sh2_main.c`. Keep this mailbox separate from the Sega-CD supervisor COMM registers; asset/CD/audio requests still own `MARS_SYS_COMM0..6`.
