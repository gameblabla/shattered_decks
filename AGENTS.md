...
- Story mode:
  ...
  - PC-FX fade-to-black "one frame fully lit before black" glitch: `apply_black_dither_fade()` on PC-FX records a palette fade level only, but when the fade reaches fully black (`visible <= 0`) it must ALSO `clear_screen(IDX_BLACK)`, exactly like the host dither path (`threshold >= 64`). A palette-only black leaves the faded-out 8bpp scene sitting in the framebuffer; the next state restores the palette to full before its first frame is uploaded, flashing that stale scene fully lit for one frame. Clearing makes every fade-to-black end on a genuinely cleared framebuffer (deck editor, name-entry, fire-dialogue, etc.).
