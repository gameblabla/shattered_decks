...
- Story mode:
  ...
  - PC-FX Load Story tries internal Backup RAM directly, falls back to FX-BMP if internal has no save, then enters `WAIFU_I_STORY_LOAD_TO_MAP`: a short visible BACKUP RAM LOADING, music-silent transition that clears the title VDC overlay before BackupRAM is read and the first story-map 3D frame is drawn.
  - The Sanctum menu (`draw_story_pyramid_menu`) uses a right-side panel so the 3D pyramid remains visible on the left. A centered opaque panel hides the PC-FX view because the PC-FX sky path is black and the pyramid sits near the center.
