...
- [Makefile.cd32x](/home/anonymous/Documents/DEV/Anime_card/waifu_card_game/Makefile.cd32x:1)
  ...
  - CD32X 3D raster hot paths live in the renderer backend split: `src/engine/renderer3d_cd32x.c` includes `renderer3d_spans.inc` with `CFX_RENDERER_USE_SH1_ASM`, while geometry and edge walking stay in `renderer3d.c`. Keep SH-2 assembly behind renderer port macros so PC-FX/headless keep their existing paths. The battle/story 3D quads use the compact affine renderer on CD32X; avoid falling back to the old per-pixel software triangle mapper unless the visual tradeoff is intentional.
