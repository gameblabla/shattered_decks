...
- [Makefile.cd32x](/home/anonymous/Documents/DEV/Anime_card/waifu_card_game/Makefile.cd32x:1)
  ...
  - CD32X rendering follows Blastem's `32x_video.c` page model: framebuffer writes always hit `video->back`, changing FS swaps `front`/`back`, and `0x24020000` is the overwrite aperture for the same back page, not page 1. Common CD32X drawing uses the current back page directly as `waifu_fm_framebuffer()`, so `waifu_cd32x_video_present_8bpp()` refreshes the line table/palette and must not repack-copy that framebuffer onto itself. Title/menu use the CD32X hardware-overlay hook again, but the backend warms both flipped pages after each title/menu mode change and restores only the prompt/menu rectangles from the resident title asset; do not return to a one-page title/menu composition or fixed physical page pointers.
