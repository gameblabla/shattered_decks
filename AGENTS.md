...
- Story mode:
  ...
  - PC-FX Sanctum screens (`draw_story_pyramid_menu`, save status, save device picker) do not render the story 3D scene. They request a VDC-only 2D background from `waifu_pcfx_video_request_vdc_background`: solid blue sky, sand ground, and transparent tile windows over the CPU-drawn panels/text. Keep the normal story map 3D path separate.
